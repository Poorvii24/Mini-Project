...
def load_model():
    ...
    # --- FIX: Look for model in current directory ---
    model_path = "transformer_classifier.pt"
    
    # Debug: Check if file exists, if not check 'models' folder
    if not os.path.exists(model_path):
        if os.path.exists("models/transformer_classifier.pt"):
            model_path = "models/transformer_classifier.pt"
        elif os.path.exists("transformer_classifier .pt"): # Check for typo with space
             model_path = "transformer_classifier .pt"
        else:
            st.error(f"❌ CRITICAL ERROR: Model file '{model_path}' not found in {os.getcwd()}")
            st.warning("Please make sure you uploaded 'transformer_classifier.pt' to your GitHub.")
            st.stop()
